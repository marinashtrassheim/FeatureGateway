...
class V3FeatureRepository:
    """Хранилище v3: msgpack (оптимизированные ключи/значения)."""
        ...
        if isinstance(row, dict):
            ...
            for name in cols:
                ...
                if raw_value is None:
                    short = PERS_ITEM_SHORT_NAMES.get(name)
                    raw_value = row.get(short, 0)
                out.append(V3FeatureRepository._restore_scaled_value(name, raw_value, multipliers))
            return out
        if isinstance(row, list):
            ...
            for i, v in enumerate(row):
                if i >= len(cols):
                    out.append(v)
                    continue
                out.append(V3FeatureRepository._restore_scaled_value(cols[i], v, multipliers))
            return out
...
TestRepositoryV3 = V3FeatureRepository